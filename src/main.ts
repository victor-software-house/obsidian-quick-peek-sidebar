...
export default class OpenSidebarHover extends Plugin {
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      this.registerDomEvent(rootSplitEl, 'mouseenter', (event: MouseEvent) => {
        // Only collapse when the pointer is genuinely over the root editing
        // area. In overlay layouts the sidebar sits on top of rootSplit as a
        // DOM sibling, so rootSplit receives mouseenter even when the pointer
        // is visually over a sidebar.
        const el = document.elementFromPoint(event.clientX, event.clientY);
        if (!el?.closest('.workspace-split.mod-vertical.mod-root')) return;
  ...
  rightSplitMouseLeaveHandler = (event: MouseEvent) => {
    // Only collapse when leaving to the root editing area or off-window.
    // If moving to any other element (ribbon, trigger zone, menu, etc.) stay open.
    const target = event.relatedTarget as HTMLElement;
    const leavingToRoot = target?.closest('.workspace-split.mod-vertical.mod-root');
    const leavingWindow = !target;
    if (!leavingToRoot && !leavingWindow) return;
