...
export default class OpenSidebarHover extends Plugin {
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      this.registerDomEvent(rootSplitEl, 'mouseenter', (event: MouseEvent) => {
        // In overlay mode the expanded sidebar sits on top of rootSplit as a
        // sibling element, so rootSplit still receives mouseenter even though
        // the mouse is visually over the sidebar. Use elementFromPoint to
        // verify the pointer isn't actually over a sidebar before collapsing.
        const el = document.elementFromPoint(event.clientX, event.clientY);

        const overLeft = el && (
          el.closest('.workspace-split.mod-horizontal.mod-sidedock.mod-left-split') ||
          el.closest('.workspace-ribbon.side-dock-ribbon.mod-left') ||
          el.closest('.workspace-tabs.mod-top.mod-top-left-space')
        );
        const overRight = el && (
          el.closest('.workspace-split.mod-horizontal.mod-sidedock.mod-right-split') ||
          el.closest('.workspace-tabs.mod-top.mod-top-right-space') ||
          el.closest('.workspace-ribbon.side-dock-ribbon.mod-right') ||
          this.rightTriggerZoneEl?.contains(el)
        );

        if (this.settings.leftSidebar && !this.isPinnedLeft && !overLeft) {
          this.isHoveringLeft = false;
          this.leftSplit?.containerEl?.removeClass('hovered');
          this.collapseLeft();
        }
        if (this.settings.rightSidebar && !this.isPinnedRight && !overRight) {
          this.isHoveringRight = false;
          this.rightSplit?.containerEl?.removeClass('hovered');
          this.collapseRight();
        }
  ...
  rightSplitMouseLeaveHandler = (event: MouseEvent) => {
    // Don't process if we're leaving to a related right-sidebar element
    const target = event.relatedTarget as HTMLElement;
    if (target && (target.closest('.workspace-split.mod-horizontal.mod-sidedock.mod-right-split') ||
                  target.closest('.workspace-tabs.mod-top.mod-top-right-space') ||
                  target.closest('.workspace-ribbon.side-dock-ribbon.mod-right') ||
                  target.closest('.workspace-tab-header-container-inner') ||
                  this.rightTriggerZoneEl?.contains(target) ||
                  target.closest('.menu'))) {
      return;
    }
  ...
  leftSplitMouseLeaveHandler = (event: MouseEvent) => {
    // Don't process if we're leaving to a related left-sidebar element
    const target = event.relatedTarget as HTMLElement;
    if (target && (target.closest('.workspace-split.mod-horizontal.mod-sidedock.mod-left-split') ||
                  target.closest('.workspace-ribbon.side-dock-ribbon.mod-left') ||
                  target.closest('.workspace-tabs.mod-top.mod-top-left-space') ||
                  target.closest('.workspace-tab-header-container-inner') ||
                  target.closest('.menu'))) {
      return;
    }
